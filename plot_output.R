...
# l <- layout.fruchterman.reingold(am.g, repulserad=vcount(am.g)^10,
#                                  area=vcount(am.g)^5)
...
idx_match = match(sim_health_ids[,sims+1], V(am.g)$name)
...
#set colour of sexes
V(am.g)$color=V(am.g)$health #assign the "Sex" attribute as the vertex color
V(am.g)$color=gsub("1","blue",V(am.g)$color) #Females will be orange
V(am.g)$color=gsub("2","red",V(am.g)$color) #Males will be lightblue
...
color_vector=V(am.g)$health #assign the "Sex" attribute as the vertex color
color_vector=gsub("1","red",color_vector) #Females will be orange
